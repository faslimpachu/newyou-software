...
import {
  existingPatients as seedPatients,
  generateMR as clientGenerateMR,
  centersByPrefix,
  type ExistingPatient,
  type ConsultationCenter,
  centerOptions,
  searchFields,
...
function Field({ label, value, onChange, type = 'text', placeholder, error, inputMode, id }: { label: string; value: string; onChange: (v: string) => void; type?: string; placeholder?: string; error?: string; inputMode?: React.HTMLAttributes<HTMLInputElement>['inputMode']; id?: string }) {
  ...
  return <div className="space-y-1.5"><Label className="text-xs font-medium text-muted-foreground" htmlFor={inputId}>{label}</Label><Input id={inputId} value={value} type={type} inputMode={inputMode} aria-invalid={!!error} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} />{error && <p className="text-xs text-destructive">{error}</p>}</div>
}
...
function mapApiPatientToExisting(patient: any, visits: any[] = []): ExistingPatient {
  const dobStr = patient.dob ? new Date(patient.dob).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : ''
  return {
    mr: patient.mr,
    ...
    lastVisit: dobStr || 'Not recorded',
    tags: patient.status === 'Visited' ? ['Registered'] : ['New'],
    visits: visits.map((v: any) => ({
      id: v.id,
      date: v.appointmentDate ? new Date(v.appointmentDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : '—',
      center: centersByPrefix[patient.mr?.startsWith('NU') ? 'NU' : 'AY'] || 'Center',
      doctor: v.doctor || v.dietitian || '—',
      reason: 'Consultation',
    })),
    bills: [],
  }
...
export function PatientWorkspace() {
  ...
  const searchPatients = useCallback(async (q: string) => {
    ...
    try {
      ...
      params.set(searchField === 'mr' ? 'search' : searchField === 'name' ? 'search' : searchField === 'mobile' ? 'search' : 'search', q)
      if (searchField === 'mr') params.set('search', q)
      else if (searchField === 'mobile') params.set('search', q.replace(/\s/g, ''))
      else if (searchField === 'name') params.set('search', q)
      else if (searchField === 'parent') params.set('search', q)

      const res = await fetch(`${API_BASE}/api/patients?${params.toString()}`)
      if (!res.ok) throw new Error('Failed to search patients')
      const data = await res.json()
      const mapped = (data.patients || []).map((p: any) => mapApiPatientToExisting(p, p.visits || []))
      setPatients(mapped)
    } catch (err: any) {
  ...
  }, [query, searchField, searchPatients])

  const matches = useMemo(() => {
    if (!query.trim()) return []
    const q = query.toLowerCase().replace(/\s/g, '')
    return patients.filter((p) => {
      const source = { mr: p.mr, mobile: p.mobile, name: p.name, parent: p.parentName }[searchField] || ''
      return source.toLowerCase().replace(/\s/g, '').includes(q)
    })
  }, [query, searchField, patients])

  const handleSelectPatient = async (patient: ExistingPatient) => {
    try {
      setLoading(true)
      setError('')
      const res = await fetch(`${API_BASE}/api/patients/${encodeURIComponent(patient.mr)}`)
      if (!res.ok) throw new Error('Failed to fetch patient details')
      const data = await res.json()
      const fullVisits = (data.patient.visits || []).map((v: any) => ({
        id: v.id,
        date: v.appointmentDate ? new Date(v.appointmentDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : '—',
        center: centersByPrefix[data.patient.mr?.startsWith('NU') ? 'NU' : 'AY'] || 'Center',
        doctor: v.doctor || v.dietitian || '—',
        reason: 'Consultation',
      }))
      const fullPatient = mapApiPatientToExisting(data.patient, fullVisits)
      setSelected(fullPatient)
      setScreen('profile')
    } catch (err: any) {
  ...
  const handleRegister = async () => {
    ...
    setSubmitting(true)
    try {
      const consultationType = center === 'nutrition' ? 'NUTRITION' : 'AYURCARE'
      const body = {
        consultationType,
        ...
        age: form.dob ? Math.floor((Date.now() - new Date(form.dob).getTime()) / (365.25 * 24 * 60 * 60 * 1000)) : undefined,
      ...
      const data = await res.json()
      const created = mapApiPatientToExisting(data.patient, [])
      created.lastVisit = new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })
      created.tags = ['New']
      handlePatientCreated(created)
      setSaved(true)
    } catch (err: any) {
  ...
  return <div className="mx-auto max-w-[1500px] pb-24">
    ...
    {screen === 'search' && <SearchPanel center={activeCenter?.name ?? ''} searchField={searchField} setSearchField={setSearchField} query={query} setQuery={setQuery} matches={matches} loading={loading} onSelect={handleSelectPatient} onNew={() => { setGeneratedMR(''); setForm(emptyForm); setFormErrors({}); setScreen('form') }} />}
    {screen === 'form' && <RegistrationForm form={form} set={set} center={center} saved={saved} submitting={submitting} onCancel={() => setScreen('search')} onSave={() => { setSaved(true); setTimeout(() => setScreen('search'), 600) }} onPatientCreated={handlePatientCreated} patients={patients} errors={formErrors} onSubmit={handleRegister} />}
...
function SearchPanel({ center, searchField, setSearchField, query, setQuery, matches, loading, onSelect, onNew }: { center: string; searchField: string; setSearchField: (s: string) => void; query: string; setQuery: (s: string) => void; matches: ExistingPatient[]; loading: boolean; onSelect: (p: ExistingPatient) => void; onNew: () => void }) {
  return <div className="grid max-w-5xl gap-6 lg:grid-cols-[1.3fr_.7fr]"><Card className="rounded-lg shadow-sm"><CardHeader><CardTitle>Find patient</CardTitle><CardDescription>Search the patient registry before creating a new medical record.</CardDescription></CardHeader><CardContent><div className="flex flex-wrap gap-2">{searchFields.map((field) => <button key={field.id} onClick={() => { setSearchField(field.id); setQuery('') }} className={cn('rounded-md border px-3 py-1.5 text-xs font-medium', searchField === field.id ? 'border-primary bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:bg-muted')}>{field.label}</button>)}</div><div className="relative mt-5"><Search className="absolute left-3 top-2.5 size-4 text-muted-foreground"/><Input className="h-9 pl-9" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={searchFields.find((item) => item.id === searchField)?.placeholder}/></div>{query && <div className="mt-4 overflow-hidden rounded-lg border">{loading ? <div className="p-8 text-center text-sm text-muted-foreground">Searching...</div> : matches.length ? matches.map((patient) => <button className="flex w-full items-center justify-between border-b p-4 text-left last:border-0 hover:bg-muted/60" key={patient.mr} onClick={() => onSelect(patient)}><div><p className="font-medium">{patient.name} <span className="ml-1 text-xs font-normal text-muted-foreground">{patient.mr}</span></p><p className="mt-1 text-xs text-muted-foreground">{patient.age} years, {patient.gender} · {patient.mobile} · {patient.city}</p></div><ChevronRight className="size-4 text-muted-foreground"/></button>) : <div className="p-8 text-center"><p className="font-medium">No patient found</p><p className="mt-1 text-sm text-muted-foreground">Create a new medical record for this patient.</p><Button className="mt-4" size="sm" onClick={onNew}><UserPlus className="mr-2 size-4"/>New registration</Button></div>}</div>}</CardContent></Card><Card className="rounded-lg bg-primary text-primary-foreground"><CardContent className="p-6"><HeartPulse className="size-8 mb-4 opacity-80"/><p className="text-sm font-medium opacity-90">{center}</p><p className="mt-1 text-xs opacity-70">Search by MR, mobile number, or patient name to continue an existing record. Create a new registration if no match is found.</p><Button className="mt-6 w-full" variant="secondary" size="sm" onClick={onNew}><UserPlus className="mr-2 size-4"/>New registration</Button></CardContent></Card></div>
}
...
function RegistrationForm({ form, set, center, saved, submitting, onCancel, onSave, onPatientCreated, patients, errors, onSubmit }: RegistrationFormProps) {
  const mrPreview = center ? clientGenerateMR(center, patients) : '--'
