...
function Field({ label, value, onChange, type = 'text', placeholder, error, inputMode, id, readOnly }: { label: string; value: string; onChange: (v: string) => void; type?: string; placeholder?: string; error?: string; inputMode?: React.HTMLAttributes<HTMLInputElement>['inputMode']; id?: string; readOnly?: boolean }) {
  return <div className="space-y-1.5"><Label htmlFor={id} className="text-xs font-medium text-muted-foreground">{label}</Label><Input id={id} value={value} type={type} inputMode={inputMode} aria-invalid={!!error} placeholder={placeholder} readOnly={readOnly} onChange={(e) => onChange(e.target.value)} />{error && <p className="text-xs text-destructive">{error}</p>}</div>
}
...
function SearchPanel({ center, searchField, setSearchField, query, setQuery, matches, loading, error, onSelect, onNew }: { center: string; searchField: string; setSearchField: (s: string) => void; query: string; setQuery: (s: string) => void; matches: PatientRecord[]; loading: boolean; error: string; onSelect: (p: PatientRecord) => void; onNew: () => void }) {
  return <div className="grid max-w-5xl gap-6 lg:grid-cols-[1.3fr_.7fr]"><Card className="rounded-lg shadow-sm"><CardHeader><CardTitle>Find patient</CardTitle><CardDescription>Search the patient registry before creating a new medical record.</CardDescription></CardHeader><CardContent><div className="flex flex-wrap gap-2">{searchFields.map((field) => <button key={field.id} onClick={() => { setSearchField(field.id); setQuery('') }} className={cn('rounded-md border px-3 py-1.5 text-xs font-medium', searchField === field.id ? 'border-primary bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:bg-muted')}>{field.label}</button>)}</div><div className="relative mt-5"><Search className="absolute left-3 top-2.5 size-4 text-muted-foreground"/><Input className="h-9 pl-9" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={searchFields.find((item) => item.id === searchField)?.placeholder}/></div>{loading && <p className="mt-3 text-sm text-muted-foreground">Loading patients...</p>}{error && <p className="mt-3 text-sm text-destructive">{error}</p>}{query && <div className="mt-4 overflow-hidden rounded-lg border">{matches.length ? matches.map((patient) => <button className="flex w-full items-center justify-between border-b p-4 text-left last:border-0 hover:bg-muted/60" key={patient.mr} onClick={() => onSelect(patient)}><div><p className="font-medium">{patient.name} <span className="ml-1 text-xs font-normal text-muted-foreground">{patient.mr}</span></p><p className="mt-1 text-xs text-muted-foreground">{patient.age} years, {patient.gender} · {patient.mobile} · {patient.city}</p></div><ChevronRight className="size-4 text-muted-foreground"/></button>) : <div className="p-8 text-center"><p className="font-medium">No patient found</p><p className="mt-1 text-sm text-muted-foreground">Create a new medical record for this patient.</p><Button className="mt-4" size="sm" onClick={onNew}><UserPlus className="mr-2 size-4"/>New registration</Button></div>}</div>}</CardContent></Card><Card className="rounded-lg shadow-sm bg-teal-600 text-white"><CardContent className="p-6"><div className="mb-5 flex size-10 items-center justify-center rounded-full bg-white/10"><HeartPulse className="size-5"/></div><p className="text-xs font-medium uppercase tracking-wider text-white/70">Selected department</p><h3 className="mt-2 font-display text-lg font-semibold">{center}</h3><p className="mt-3 text-sm leading-6 text-white/80">A unique MR number is issued automatically after a successful registration.</p><Button className="mt-6 w-full bg-white text-teal-700 hover:bg-white/90" size="sm" onClick={onNew}><UserPlus className="mr-2 size-4"/>Register new patient</Button></CardContent></Card></div>
}
...
function RegistrationForm({ form, set, center, centerName, existingPatient, saved, onCancel, onSave, onError }: { form: FormValues; set: (k: string) => (v: string) => void; center: ConsultationCenter | null; centerName: string; existingPatient: PatientRecord | null; saved: boolean; onCancel: () => void; onSave: (patient: PatientRecord) => void; onError: (message: string) => void }) {
  ...
  const validate = () => { const next: Record<string, string> = {}; if (!form.firstName.trim()) next.firstName = 'Patient name is required.'; if (!form.gender) next.gender = 'Gender is required.'; if (!/^\d{10}$/.test(form.mobile)) next.mobile = 'Enter exactly 10 numeric digits.'; if (form.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) next.email = 'Enter a valid email address.'; if (!form.address.trim()) next.address = 'Address is required.'; if (!form.city.trim()) next.city = 'District is required.'; if (!form.state.trim()) next.state = 'State is required.'; if (!/^\d{6}$/.test(form.postalCode)) next.postalCode = 'Enter a valid 6 digit PIN code.'; if (!form.doctor) next.doctor = 'Please select a consulting doctor.'; setErrors(next); return Object.keys(next).length === 0 }
  useEffect(() => { if (form.dob) set('age')(String(computeAge(form.dob))) }, [form.dob])
  ...
  const select = (label: string, key: string, values: string[], error?: string) => {
    const id = `select-${key}`
    return <div className="space-y-1.5"><Label htmlFor={id} className="text-xs font-medium text-muted-foreground">{label}</Label><select id={id} value={form[key]} onChange={(e) => set(key)(e.target.value)} className="h-8 w-full rounded-lg border border-input bg-background px-2.5 text-sm"><option value="">Select</option>{values.map((v) => <option key={v} value={v}>{v}</option>)}</select>{error && <p className="-mt-2 text-xs text-destructive">{error}</p>}</div>
  }
  return <><div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-primary/20 bg-primary/5 px-4 py-3 text-sm">{existingPatient ? <span><span className="font-semibold text-primary">New visit for {existingPatient.name}.</span> Saving creates a new backend visit.</span> : <span><span className="font-semibold text-primary">New patient record.</span> MR number is issued by the server after successful registration.</span>}<span className="rounded bg-background px-2 py-1 font-mono text-xs text-primary">MR: {existingPatient?.mr ?? 'Generated on save'}</span></div><div className="mb-4 flex items-center gap-2 text-sm text-muted-foreground"><span className="font-medium">Center:</span><span className="rounded-md border border-primary/20 bg-primary/5 px-2.5 py-1 text-xs font-semibold text-primary">{centerName}</span></div><div className="grid gap-5 xl:grid-cols-2"><Section title="Consulting Doctor" description="Choose from the doctors available for this department"><div className="space-y-1.5"><Label className="text-xs font-medium text-muted-foreground">Consulting doctor *</Label><select value={form.doctor} onChange={(e) => update('doctor')(e.target.value)} className="h-8 w-full rounded-lg border border-input bg-background px-2.5 text-sm"><option value="">Select a doctor</option>{doctorOptions.map((doc) => <option key={doc.id} value={doc.name}>{doc.name} - {doc.qualification}</option>)}</select>{errors.doctor && <p className="text-xs text-destructive">{errors.doctor}</p>}<p className="text-xs text-muted-foreground">Showing {doctorOptions.length} doctors for {centerName}.</p></div></Section><Section title="Personal Information" description="Fields marked * are required"><div className="grid gap-4 sm:grid-cols-2"><Field id="firstName" label="First name *" value={form.firstName} onChange={update('firstName')} error={errors.firstName}/><Field id="lastName" label="Last name" value={form.lastName} onChange={update('lastName')} /><Field id="dob" label="Date of birth" value={form.dob} type="date" onChange={update('dob')} /><Field id="age" label="Age" value={form.age} onChange={update('age')} readOnly />{select('Gender *', 'gender', ['Male','Female','Other'], errors.gender)}{select('Blood group', 'bloodGroup', ['A+','A-','B+','B-','O+','O-','AB+','AB-'])}<Field id="parentName" label="Parent / spouse name" value={form.parentName} onChange={update('parentName')} error={errors.parentName}/></div></Section><Section title="Contact Information"><div className="grid gap-4 sm:grid-cols-2"><Field id="mobile" label="Mobile number *" value={form.mobile} onChange={update('mobile')} inputMode="numeric" error={errors.mobile} placeholder="10 digit mobile number"/><Field id="email" label="Email address" value={form.email} onChange={update('email')} type="email" error={errors.email}/></div></Section><Section title="Address"><div className="grid gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><Field id="address" label="Street address *" value={form.address} onChange={update('address')} error={errors.address}/></div><Field id="city" label="District *" value={form.city} onChange={update('city')} error={errors.city}/><Field id="state" label="State *" value={form.state} onChange={update('state')} error={errors.state}/><Field id="postalCode" label="PIN code *" value={form.postalCode} onChange={update('postalCode')} inputMode="numeric" error={errors.postalCode}/></div></Section><Section title="Emergency Contact"><div className="grid gap-4 sm:grid-cols-3"><Field id="emergencyName" label="Contact name" value={form.emergencyName} onChange={update('emergencyName')} /><Field id="emergencyPhone" label="Phone number" value={form.emergencyPhone} onChange={update('emergencyPhone')} inputMode="numeric"/><Field id="emergencyRelation" label="Relationship" value={form.emergencyRelation} onChange={update('emergencyRelation')} /></div></Section><Section title="Medical History" description="Document known conditions before consultation"><div className="grid gap-4"><div><Label className="text-xs text-muted-foreground">Allergies</Label><Textarea className="mt-1.5 min-h-18" value={form.allergies} onChange={(e) => update('allergies')(e.target.value)} /></div><div><Label className="text-xs text-muted-foreground">Existing conditions</Label><Textarea className="mt-1.5 min-h-18" value={form.conditions} onChange={(e) => update('conditions')(e.target.value)} /></div><Field id="medications" label="Current medications" value={form.medications} onChange={update('medications')} /></div></Section><Section title="Lifestyle"><div className="grid gap-4 sm:grid-cols-2">{select('Smoking', 'smoking', ['Never','Occasional','Regular','Former'])}{select('Alcohol', 'alcohol', ['Never','Occasional','Regular','Former'])}{select('Exercise', 'exercise', ['Sedentary','Light','Moderate','Active','Very Active'])}{select('Diet pattern', 'diet', ['Vegetarian','Non-Vegetarian','Eggetarian','Vegan','Jain'])}</div></Section></div><div className="fixed bottom-0 left-0 right-0 z-30 border-t bg-background/95 px-4 py-3 backdrop-blur md:left-64"><div className="mx-auto flex max-w-[1500px] justify-end gap-2"><Button variant="outline" onClick={onCancel}><X className="mr-2 size-4"/>Cancel</Button><Button disabled={saved} onClick={handleSave}>{saved ? <Check className="mr-2 size-4"/> : <UserPlus className="mr-2 size-4"/>}{saved ? 'Registration created' : existingPatient ? 'Save visit' : 'Save & create visit'}</Button></div></div></>
}