...
import {
  ...
  Pencil, Plus, Printer, Stethoscope, Trash2, Upload, X, Speaker,
} from 'lucide-react'
...
export function PatientProfile({ patient, center, generatedMR, justRegistered, onNewVisit }: PatientProfileProps) {
  ...
  const speakPatient = () => {
    ...
    const text = `Patient ${p.name}, MR ${p.mr}, Age ${p.age || 'not specified'}, Gender ${p.gender || 'not specified'}, Blood group ${p.bloodGroup || 'not recorded'}, Phone ${p.mobile || 'not recorded'}, Address ${p.address || 'not recorded'}, ${p.city || ''} ${p.state || ''} ${p.postalCode || ''}`
...
const PRINT_PAGE_CSS = '@page{size:A5 portrait;margin:10mm}'
