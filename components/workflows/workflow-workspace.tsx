...
function dateInputValue(value?: string | number | null) {
...
export function WorkflowWorkspace(_props: { mode?: string } = {}) {
  ...
  const loadFollowUps = useCallback(async (showLoading = true) => {
    ...
    try {
      ...
      setSelected((current) => {
        if (showLoading || !current) return mapped[0] ?? null
